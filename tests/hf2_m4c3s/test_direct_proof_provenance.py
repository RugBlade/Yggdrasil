"""M4C3S focused tests against installed candidate runtime bytes.

No source assembly, legacy infer oracle, immutable extraction or completed
prototype runner is executed here. Fixtures retain the frozen behavioral cases.
"""
...
CASES=['integrated_why_has_two_routes_and_four_supports', 'real_reasoning_preserves_02_09_across_24_orders', 'integrated_atomic_parallel_routes', 'integrated_transitive_parallel_routes', 'integrated_universal-positive_parallel_routes', 'integrated_universal-negative_parallel_routes', 'single_route_zero_is_numeric_zero_not_default_or_absence', 'direct_target_preserves_derivations_bare_truth_is_not_explanation', 'cyclic_nested_symbolic_routes_pass_models_and_conservative_audit', 'reasoning_duplicate_observations_do_not_multiply_proof_records', 'response_autonomous_and_serialization_preserve_undefined_confidence', 'proof_budget_incompleteness_reaches_both_content_consumers', 'closure_budget_incompleteness_reaches_both_content_consumers', 'public_infer_four_item_compatibility_and_separate_max_steps_budget', 'typed_universal_collision_keeps_provenance_and_surface_options_distinct', 'all_routes_missing_remains_blocked', 'all_routes_conflict_remains_blocked', 'all_routes_scoped_remains_blocked', 'all_routes_unscoped_remains_blocked', 'autonomous_path_uses_same_all_route_conservative_audit', 'parallel_current_bounded_DKT_sources_and_native_admission_preserved', 'existing_direct_and_reasoning_correction_admission_matches_ten_controls', 'fourteen_public_query_families_keep_observed_content_contract', 'genuine_proposition_alternatives_stay_act_neutral', 'two_complete_Noeron_ingest_orders_and_state_roundtrip', 'integrated_proof_inventory_native_blank', 'integrated_proof_inventory_native_tied', 'integrated_proof_inventory_native_missing', 'integrated_proof_inventory_native_learned-clarify', 'integrated_proof_inventory_native_learned-ask', 'integrated_proof_inventory_native_learned-silent', 'owner_calibration_preserves_operator_boundary', 'sealed_0022_surface_correspondence_with_typed_proof_metadata', 'permanent_candidate_boundary_and_authority']

@pytest.fixture(scope='module')
def candidate_checks(tmp_path_factory):
    td=tmp_path_factory.mktemp('m4c3s-candidate')
    checks=[]
    def check(name,fn,category='runtime_compatibility'):
        try:
            evidence=fn()
            checks.append({'id':name,'category':category,'status':'PASS','evidence':evidence})
        except Exception:
            checks.append({'id':name,'category':category,'status':'FAIL','error':traceback.format_exc()})
    from noeron.direct_proof import infer_compatible,gate_compatible,typed_id,admit
    from noeron.inference import parse_surface_logic_detailed,proposition_key,infer
    ...
    for mode,kwargs in [('proof',{'max_proof_records':1}),('closure',{'max_conclusion_steps':0})]:
    ...
    def corrections():
        ...
        for mode in ('direct','reasoning'):
            expectations = ([{key(new)}, {key(old),key(new)}, {key(old),key(new)}, {key(new)}, {key(cases[4][0]),key(new)}]
                if mode=='direct' else [{key(new)}, {key(old),key(new)}, {key(new)}, {key(old),key(new)}, {key(cases[4][0]),key(new)}])
            for index,pp in enumerate(cases):
                r=infer(pp,q,admission_mode=mode)
                assert {key(p) for p in r.candidates}==expectations[index]
    ...
    def query_compatibility():
        ...
        for kind in ('derive-all','relation-between','relation-from-subject','relation-to-object','exact-proposition',
            ...
            expected_indexes={'derive-all':[], 'relation-between':[0], 'relation-from-subject':[0,2,3,4,5],
                'relation-to-object':[0], 'exact-proposition':[0], 'why-proposition':[],
                'copular-value-from-subject':[0], 'relation-subjects':[0,1,2,3,4],
                'where-from-subject':[2], 'when-from-subject':[3], 'how-from-subject':[4],
                'none':[], 'unresolved-surface-question':[], 'unresolved-reference':[]}[kind]
            new=infer(pp,q)
            assert {key(p) for p in new[2]}=={key(pp[i]) for i in expected_indexes},kind
            expected_status=('no-logical-selection-requested' if kind in {'none','unresolved-surface-question','unresolved-reference'}
                else 'no-proof-supported-answer' if not expected_indexes else 'unique-proof-supported-answer' if len(expected_indexes)==1
                else 'multiple-proof-supported-answers-no-forced-choice')
            assert new.status==expected_status,kind
    ...
    def permanent_boundary():
        import inspect
        import noeron.inference as inference_module
        import noeron.reasoning as reasoning_module
        assert not hasattr(inference_module,'_legacy_infer_for_disposable_comparison')
        assert 'max_conclusion_steps' in inspect.signature(reasoning_module.build_multi_memory_reasoning).parameters
        assert 'max_proof_records' in inspect.signature(reasoning_module.build_multi_memory_reasoning).parameters
        assert not any(integrated.proof_ledger['authority'].values())
        assert not any(integrated.proof_gate_authority.values())
        return {'comparison_oracle_removed':True,'permanent_budget_parameters':True,'authority_false':True}
    check('permanent_candidate_boundary_and_authority',permanent_boundary,'provenance')
    return {row['id']:row for row in checks}
...
def test_frozen_direct_proof_contract(candidate_checks,case):
    assert set(candidate_checks)==set(CASES)
    row=candidate_checks[case]
    assert row['status']=='PASS',row.get('error',row)
